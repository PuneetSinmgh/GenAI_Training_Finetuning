...
# Bert based tokenizer and model
...
embeddings_yes_responses=text_to_emb(yes_responses)
tsne_plot(embeddings_yes_responses, "Yes Response")
...
embeddings_sesame_street=text_to_emb(sesame_street)
...
songs = [song_rage, sesame_street, my_shoe_lyrics]
embeddings = [text_to_emb(song) for song in songs]
all_embeddings = np.vstack(embeddings)

#######
###### DOT Product Similarity for RAG Vectors

# The dot product of two vectors, and , each with components  and , is a fundamental operation in linear algebra calculated as follows:
...
#Now, consider the matrix A , where each row is the RAG embeddings. In the context of matrix multiplication involving a dot product, if is a matrix and is a vector, then  computes the dot product of 
# with each row of. In this scenario, each vector in A is treated as a row vector. Here, 
# represents embeddings_questions, while embeddings_rage is the RAG vector . The output is a column tensor that measures how closely each RAG vector for the questions aligns with the song.
...
RAG_QA(embeddings_questions, embeddings_yes_responses, n_responses=3)

RAG_QA(embeddings_questions, all_embeddings, n_responses=3)
...
############## Cosine Similarity for RAG Vectors    
